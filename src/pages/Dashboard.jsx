...
import {
  Play, Pause, CheckCircle2, Lock, ChevronDown, ChevronRight,
  LogOut, BookOpen, Clock, Download, MessageSquare, Bell,
  FileText, StickyNote, Volume2, Maximize, SkipForward, SkipBack,
...
const chapters = [
  {
    id:'C1', title:'Chapter 1: Mindset', duration:'1:28:40',
    color:'text-violet-400', bg:'bg-violet-500/10', border:'border-violet-500/25', dot:'bg-violet-500',
    lessons:[
      { id:'L1', title:'Chào mừng & Lộ trình 90 ngày',           duration:'12:30', done:true,  videoUrl:'', thumb:null },
      { id:'L2', title:'Reset tư duy — tại sao kỹ năng không đủ', duration:'18:45', done:true,  videoUrl:'', thumb:null },
      { id:'L3', title:'Cách thị trường freelance vận hành',       duration:'22:10', done:true,  videoUrl:'', thumb:null },
      { id:'L4', title:'Xác định mục tiêu $1,000 đầu tiên',        duration:'15:20', done:false, videoUrl:'', thumb:null },
      { id:'L5', title:'Mindset của người kiếm tiền vs người học',  duration:'19:55', done:false, videoUrl:'', thumb:null },
    ],
  },
  {
    id:'C2', title:'Chapter 2: Skill', duration:'3:36:25',
    color:'text-blue-400', bg:'bg-blue-500/10', border:'border-blue-500/25', dot:'bg-blue-500',
    lessons:[
      { id:'L6',  title:'Premiere Pro từ A đến Z (Workflow)',    duration:'35:00', done:false, videoUrl:'', thumb:null },
      { id:'L7',  title:'Cut, trim & cấu trúc video chuẩn',      duration:'28:15', done:false, videoUrl:'', thumb:null },
      { id:'L8',  title:'Color grading cho video quốc tế',        duration:'31:40', done:false, videoUrl:'', thumb:null },
      { id:'L9',  title:'Sound design & music sync',              duration:'24:30', done:false, videoUrl:'', thumb:null },
      { id:'L10', title:'After Effects: motion graphics cơ bản', duration:'42:00', done:false, videoUrl:'', thumb:null },
      { id:'L11', title:'Xây dựng 3 video mẫu portfolio',         duration:'55:20', done:false, videoUrl:'', thumb:null },
    ],
  },
  {
    id:'C3', title:'Chapter 3: Sales', duration:'2:28:45', locked:true,
    color:'text-emerald-400', bg:'bg-emerald-500/10', border:'border-emerald-500/25', dot:'bg-emerald-500',
    lessons:[
      { id:'L12', title:'Xây portfolio Instagram thu hút client',       duration:'26:10', done:false, locked:true, videoUrl:'', thumb:null },
      { id:'L13', title:'10 Template Cold DM tiếng Anh',                duration:'33:45', done:false, locked:true, videoUrl:'', thumb:null },
      { id:'L14', title:'Phân tích profile, nhận diện khách tiềm năng', duration:'21:00', done:false, locked:true, videoUrl:'', thumb:null },
      { id:'L15', title:'Đàm phán & báo giá không bị ép giá',           duration:'29:30', done:false, locked:true, videoUrl:'', thumb:null },
      { id:'L16', title:'Giữ client dài hạn & upsell',                  duration:'18:20', done:false, locked:true, videoUrl:'', thumb:null },
    ],
  },
]
...
const discussions = [
  { id:1, user:'Minh Anh', avatar:'MA', time:'2 ngày trước', likes:12,
    text:'Mình đã áp dụng phương pháp phân tích ngược này và thực sự thấy rõ hướng đi hơn rất nhiều!',
    replies:[{ id:2, user:'Huy Trainer', avatar:'HT', time:'1 ngày trước', likes:5, isInstructor:true,
      text:'Tuyệt vời! Chia sẻ kết quả sau 30 ngày nhé.' }] },
  { id:3, user:'Kiên', avatar:'K', time:'5 ngày trước', likes:8,
    text:'Câu hỏi về rate: mới bắt đầu có nên nhận job $50 để xây portfolio không ạ?', replies:[] },
]
...
function getEmbedUrl(url) {
  ...
  if (long) return `https://www.youtube.com/embed/${long[1]}?autoplay=1&rel=0&modestbranding=1`
    ...
    <div className="relative w-full bg-black overflow-hidden" style={{aspectRatio:'16/9'}}>
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="text-center px-8">
          {embedUrl
            ? <p className="text-blue-400 text-[10px] uppercase tracking-widest mb-2 flex items-center justify-center gap-1.5"><PlayCircle size={11}/> Video thực hành</p>
          ...
          {!embedUrl && <p className="mt-2 text-[11px] text-slate-600">Chưa có video — Thêm URL trong trang Admin</p>}
        </div>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-4 pb-3 pt-8">
        <div className="mb-2 cursor-pointer" onClick={e=>{
          ...
          <div className="h-1 rounded-xl bg-white/20">
            <div className="h-full rounded-xl bg-blue-500 relative transition-all" style={{width:`${progress}%`}}>
              <div className="absolute right-0 top-1/2 -translate-y-1/2 h-3 w-3 rounded-xl bg-blue-400 shadow-lg"/>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={()=>setPlaying(v=>!v)} className="text-white hover:text-blue-300 transition-colors">
              {playing?<Pause size={15}/>:<Play size={15} fill="white"/>}
            </button>
            <button className="text-slate-400 hover:text-white"><SkipBack size={13}/></button>
            <button className="text-slate-400 hover:text-white"><SkipForward size={13}/></button>
            <span className="text-[11px] text-slate-400 tabular-nums">
              {Math.floor(progress*0.15*60/100)}:{String(Math.floor((progress*15*60/100)%60)).padStart(2,'0')} / {lesson?.duration}
            </span>
          </div>
          <div className="flex items-center gap-3">
            <button className="text-slate-400 hover:text-white"><Volume2 size={13}/></button>
            <button className="text-[11px] text-slate-400 hover:text-white font-medium">1×</button>
            <button className="text-slate-400 hover:text-white"><Maximize size={13}/></button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="rounded-xl border bg-muted/40 p-4">
          <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-line">{c.transcript}</p>
        </div>
      </div>
      <div className="flex flex-wrap gap-2">
        {['Mindset','Mục tiêu','Freelance','$1,000 đầu tiên'].map(t=>(
          <span key={t} className="rounded-xl border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-medium text-primary">{t}</span>
        ))}
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      ...
      <textarea value={note} onChange={e=>setNote(e.target.value)}
        placeholder="Ghi lại những điểm quan trọng..." rows={10}
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={()=>{setSaved(true);setTimeout(()=>setSaved(false),2000)}}
          ...
          {saved?<><CheckCircle2 size={12}/> Đã lưu</>:'Lưu ghi chú'}
        </button>
      </div>
    </div>
  )
...
function TabResources() {
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-foreground flex items-center gap-2">
        <Paperclip size={13} className="text-muted-foreground"/> Tài liệu đính kèm
      </h3>
      {lessonResources.map((r,i)=>(
        <motion.div key={r.name} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} transition={{delay:i*0.08}}
...
function TabDiscussion() {
  ...
  const post=()=>{
    if(!newQ.trim())return
    setThreads(p=>[{id:Date.now(),user:'Bạn',avatar:'B',time:'Vừa xong',likes:0,text:newQ,replies:[]},...p])
    setNewQ('')
  }
  return (
    <div className="space-y-4">
      <div className="rounded-xl border bg-card p-4">
        ...
        <textarea value={newQ} onChange={e=>setNewQ(e.target.value)}
          placeholder="Hỏi bất cứ điều gì..." rows={3}
        ...
        <div className="flex justify-end mt-2">
          <button onClick={post} className="flex items-center gap-1.5 rounded-xl bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-all">
      ...
      {threads.map(t=>(
        <div key={t.id} className="rounded-xl border bg-card p-4">
          <div className="flex items-start gap-3 mb-3">
            <div className="h-8 w-8 flex-shrink-0 rounded-xl bg-muted flex items-center justify-center text-xs font-bold text-foreground">{t.avatar}</div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-foreground">{t.user}</span>
                <span className="text-xs text-muted-foreground">{t.time}</span>
              </div>
              <p className="text-sm text-muted-foreground leading-relaxed mt-1">{t.text}</p>
            </div>
          </div>
          <div className="flex items-center gap-3 pl-11 mb-2">
            <button className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary"><ThumbsUp size={11}/> {t.likes}</button>
            <button onClick={()=>setOpenReply(openReply===t.id?null:t.id)} className="text-xs text-muted-foreground hover:text-primary">Trả lời</button>
          </div>
          {t.replies.map(r=>(
            <div key={r.id} className="ml-11 mt-3 rounded-xl border bg-muted/30 p-3 flex gap-3">
              <div className={`h-7 w-7 flex-shrink-0 rounded-xl flex items-center justify-center text-[10px] font-bold text-white ${r.isInstructor?'bg-primary':'bg-muted-foreground'}`}>{r.avatar}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-foreground">{r.user}</span>
                  {r.isInstructor&&<span className="rounded-xl bg-primary/10 border border-primary/20 px-1.5 py-0.5 text-[9px] font-bold text-primary">TRAINER</span>}
          ...
          {openReply===t.id&&(
            <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} className="ml-11 mt-3 flex gap-2">
              <input placeholder="Viết trả lời..." className="flex-1 rounded-xl border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-primary/50"/>
              <button className="rounded-xl bg-primary px-3 py-2 text-primary-foreground hover:bg-primary/90"><Send size={12}/></button>
            </motion.div>
          )}
        </div>
      ))}
...
function LessonThumb({ lesson, isActive, isDone, isLocked }) {
  return (
    <div className={`relative flex-shrink-0 w-[72px] h-[42px] rounded overflow-hidden border ${
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        {isLocked
          ...
          : isDone
            ...
            : <PlayCircle size={14} className={isActive ? 'text-blue-400' : 'text-slate-500'}/>
        }
      </div>
      {isActive && (
...
export function Dashboard({ onLogout, onAdmin, onProfile }) {
  ...
  useEffect(() => {
    ...
    const handler = e => { if (!e.target.closest('[data-avatar-menu]')) setAvatarOpen(false) }
  ...
  const activeChapter = chapters.find(c=>c.lessons.some(l=>l.id===activeLesson?.id))
  ...
  const selectLesson = l => { setActiveLesson(l); setTab('content') }
  const markDone = () => {
    ...
    <div className="flex h-screen w-full overflow-hidden bg-[#0f1117]">
      ...
      <aside className="flex flex-col w-[300px] flex-shrink-0 border-r border-white/[0.07]" style={{ background:'#141520' }}>
        ...
        <div className="flex items-center gap-3 px-4 py-3.5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-semibold text-white leading-tight truncate">VIRAL FREEDOM SYSTEM</p>
            <p className="text-[10px] text-slate-500 mt-0.5">90-Day Challenge</p>
          </div>
        </div>
            ...
            <div key={ch.id}>
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  <div className={`h-2 w-2 rounded-xl flex-shrink-0 ${ch.locked ? 'bg-slate-600' : ch.dot}`}/>
                  <div className="min-w-0">
                    <p className={`text-[12px] font-semibold leading-tight ${ch.locked ? 'text-slate-600' : 'text-slate-200'}`}>
                      {ch.title}
                    </p>
                    <p className="text-[10px] text-slate-600 mt-0.5">
                      {ch.lessons.filter(l=>doneIds.has(l.id)).length}/{ch.lessons.length} bài
                      {ch.locked && <span className="ml-1.5 inline-flex items-center gap-0.5"><Lock size={8}/> Đã khóa</span>}
                    </p>
                  </div>
                </div>
                  ...
                  >
                    {ch.lessons.map((lesson, li) => {
                      ...
                      return (
                        ...
                        >
                          {/* Thumbnail */}
                          <LessonThumb lesson={lesson} isActive={isActive} isDone={isDone} isLocked={lesson.locked}/>

                          {/* Lesson info */}
                          <div className="flex-1 min-w-0 pt-0.5">
                            <p className={`text-[12px] leading-snug font-medium ${
                              isActive ? 'text-blue-300' : isDone ? 'text-slate-500 line-through' : 'text-slate-300'
                            }`}>
                              {lesson.title}
                            </p>
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        <header className="flex-shrink-0 flex items-center justify-between px-5 py-2.5 border-b border-white/[0.07] bg-[#0f1117]">
          ...
          <div className="flex items-center gap-2 text-[12px] min-w-0">
            <span className="text-slate-600">{activeChapter?.title || 'Khoá học'}</span>
            <ChevronRight size={12} className="text-slate-700 flex-shrink-0"/>
            <span className="text-slate-300 font-medium truncate max-w-[240px]">{activeLesson?.title}</span>
          </div>
                ...
                <div style={{
                  ...
                  <div style={{borderTop:'1px solid rgba(255,255,255,0.07)'}}>
                    <button onClick={()=>{setAvatarOpen(false);onLogout()}}
                      style={{display:'flex',width:'100%',alignItems:'center',gap:10,padding:'10px 14px',fontSize:12,color:'#f87171',background:'none',border:'none',cursor:'pointer',textAlign:'left'}}
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-6 py-5 max-w-4xl">
            ...
            <div className="flex items-start justify-between gap-4 mb-5">
              <div className="min-w-0">
                {activeChapter && (
                  <span className={`inline-block mb-2 rounded-xl px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider border ${activeChapter.bg} ${activeChapter.color} ${activeChapter.border}`}>
                    {activeChapter.title}
                  </span>
                )}
                <h1 className="text-lg font-semibold text-white leading-snug">{activeLesson?.title}</h1>
                <p className="flex items-center gap-3 mt-1 text-[12px] text-slate-500">
                  ...
                  {doneIds.has(activeLesson?.id) && (
                    <span className="flex items-center gap-1 text-emerald-500"><CheckCircle2 size={10}/> Đã hoàn thành</span>
                  )}
                </p>
              </div>
            ...
            <div className="mt-8 flex gap-3">
              {prevLesson
                ? <button onClick={()=>selectLesson(prevLesson)}
                    ...
                    <ChevronLeft size={15} className="text-slate-600 group-hover:text-white flex-shrink-0 transition-colors"/>
                    <div className="min-w-0">
                      <p className="text-[10px] text-slate-600 mb-1 uppercase tracking-wide">Bài trước</p>
                      <p className="text-[13px] text-slate-300 group-hover:text-white truncate transition-colors">{prevLesson.title}</p>
                    </div>
                  </button>
              ...
              {nextLesson && (
                <button onClick={()=>selectLesson(nextLesson)}
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] text-slate-600 mb-1 uppercase tracking-wide">Bài tiếp theo</p>
                    <p className="text-[13px] text-slate-300 group-hover:text-white truncate transition-colors">{nextLesson.title}</p>
                  </div>
                  <ChevronRight size={15} className="text-slate-600 group-hover:text-blue-400 flex-shrink-0 transition-colors"/>
                </button>
              )}
            </div>
            <div className="h-10"/>
